...
chokidar.watch(OTFstor).on('add', (filepath, filestatus) => {
	...
	fs.readdir(OTFstor, (err, files) => {
		...
		for (var i = 0; i < OTF_array.length; i++) {
			...
			if (OTF_array.length > 1) {
				if (((OTF_array.length - 1) == i) && (OTF_array.length == (i + 1))) {
					fs.readFile(OTFstor.concat("/", OTF_array[i]), (err, data) => {
						...
						fs.writeFileSync(OTFfile, comma.concat(last_file, end_bracks), {encoding:'utf8', flag:'a'});
						console.log(' OTF_File.txt updated..');
					});
				}
				else if ((i > 0) && (i < (OTF_array.length - 1))) {
					fs.readFile(OTFstor.concat("/", OTF_array[i]), (err, data) => {
						//if (err) throw err;
						var comma = ", ";
            ...
            else if (OTF_array.length == 1) {
				fs.readFile(OTFstor.concat("/", OTF_array[i]), (err, data) => {
					//if (err) throw err;
					var prefix_part = "{ \"events\": [";
					var end_bracks = " ]}";
					var first_file = data.toString();
					fs.writeFileSync(OTFfile, prefix_part.concat(first_file, end_bracks), {encoding:'utf8', flag:'w'});
					console.log(' OTF_File.txt updated..');
				});
			}
